    ...
    for (const width of [1440,1366,1280,1024,768,390,320]) {
        ...
        for (const path of ['/','/login.jsp','/register.jsp']) {
            ...
            if ([1440,390].includes(width)) {
                ...
                fs.writeFileSync(`verification/premium-${theme}-${width}-${path==='/'?'home':path.slice(1,-4)}.png`,Buffer.from(shot.data,'base64'));
            }
            if(path==='/' && width<1320) {
...
fs.writeFileSync('verification/premium-browser-results.json',JSON.stringify(report,null,2));
