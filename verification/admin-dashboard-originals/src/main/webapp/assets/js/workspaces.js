...
(() => {
  ...
  const iconFor=text=>/loan/i.test(text)?'loans':/investment/i.test(text)?'investments':/card/i.test(text)?'cards':/support|ticket|request/i.test(text)?'support':/employee|customer/i.test(text)?'people':/audit|repay/i.test(text)?'audit':/cash|pay/i.test(text)?'payments':'accounts';
  ...
  function collectRecords(panel,title){
    ...
    function render(){const query=search.value.trim().toLowerCase();const filtered=index.filter(r=>(!states.value||r.state===states.value)&&(!query||r.text.includes(query)));page=Math.max(0,Math.min(page,Math.ceil(filtered.length/6)-1));tbody.replaceChildren();
    ...
    search.addEventListener('input',()=>{page=0;render();});states.addEventListener('change',()=>{page=0;render();});render();
  ...
  $$('form').filter(form=>form.method.toLowerCase()==='post'&&!form.closest('.notification-center,.notifications-page')).forEach(form=>{
    ...
    else if(action==='ticket'){node=form.closest('.panel');key='action-ticket';}
  ...
  if(staff){
    ...
    if(body.classList.contains('overview-page')){
      ...
      const launchers=el('div','workspace-launchers');for(const key of ['action-transfer','action-pay','action-schedule','action-apply','action-request','action-ticket']){const action=actions.get(key);if(action)launchers.append(action.launch);}
  ...
  function route(focus=false){const hash=decodeURIComponent(location.hash.slice(1));if(!hash.startsWith('action-')&&actionView.dialog.open)actionView.dialog.close();
