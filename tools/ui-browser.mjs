...
for(const width of [1440,1280,768,390]){
 ...
 for(const [role,path] of routes){
  ...
  if(result.overflow||result.badImages.length||result.badText)failures.push({width,role,path,...result});checks++;
  if((width===1440||width===390)&&['/','/login.jsp','/register.jsp','/customer/dashboard','/customer/transfer'].includes(path)||width===390&&role==='SYSTEM_ADMIN'){
   const shot=await call('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync('target/ui-'+width+'-'+role+'-'+(path.replaceAll('/','-')||'home')+'.png',Buffer.from(shot.data,'base64'));
  }
...
fs.writeFileSync('target/ui-browser-result.json',JSON.stringify({checks,failures},null,2));console.log(JSON.stringify({checks,failures},null,2));ws.close();
