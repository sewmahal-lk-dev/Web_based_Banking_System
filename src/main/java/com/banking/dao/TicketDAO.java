...
public class TicketDAO {
    ...
    public void update(int employee,int ticketId,String action,String next,String body)throws SQLException {
        ...
        final String requested="ticket-reply".equals(operation)?null:Input.choice(next,"IN_PROGRESS","ESCALATED","RESOLVED");
        Jdbc.transaction(c->{
            String role=staff(c,employee);
            var ticket=Jdbc.one(c,"SELECT status,assigned_role FROM ticket WHERE ticket_id=? FOR UPDATE",ticketId);
            ...
            if(!List.of("OPEN","ASSIGNED","IN_PROGRESS","ESCALATED").contains(state))throw new IllegalArgumentException("Resolved or closed tickets cannot be changed.");
