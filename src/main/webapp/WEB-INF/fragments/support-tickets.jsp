...
<p><a class="btn" href="#new-complaint">Submit Complaint</a></p>
...
<p class="support-reference"><%= "COMPLAINT".equals(ticket.get("ticket_type"))?"COMPLAINT":"Ticket" %> #TKT-<%= String.format("%04d",((Number)ticket.get("ticket_id")).intValue()) %></p>
...
<% for(boolean complaintForm:List.of(false,true)){ %>
<section class="panel" id="<%= complaintForm?"new-complaint":"new-support-ticket" %>"><h2><%= complaintForm?"Submit Complaint":"New support ticket" %></h2>
<p>Your submission and staff replies will appear in your support history.</p>
<form method="post"><input type="hidden" name="csrf" value="<%= Input.html(session.getAttribute("csrf")) %>"><input type="hidden" name="action" value="ticket">
<% if(complaintForm){ %><input type="hidden" name="type" value="COMPLAINT"><p>Category: <span class="status-badge">COMPLAINT</span></p>
<% }else{ %><label>Type<select name="type"><% for(String type:List.of("INQUIRY","COMPLAINT","TECHNICAL","OTHER")){ %><option><%= type %></option><% } %></select></label><% } %>
<label>Subject<input name="subject" maxlength="150" value="<%= Input.html(request.getParameter("subject")) %>" required></label>
<label>Description<textarea name="description" maxlength="5000" required><%= Input.html(request.getParameter("description")) %></textarea></label>
<label>Priority<select name="priority"><% for(String priority:complaintForm?List.of("LOW","MEDIUM","HIGH"):List.of("LOW","MEDIUM","HIGH","URGENT")){ %><option value="<%= priority %>" <%= priority.equals(request.getParameter("priority")==null?"MEDIUM":request.getParameter("priority"))?"selected":"" %>><%= priority %></option><% } %></select></label>
<button type="submit"><%= complaintForm?"Submit Complaint":"Submit ticket" %></button></form></section>
