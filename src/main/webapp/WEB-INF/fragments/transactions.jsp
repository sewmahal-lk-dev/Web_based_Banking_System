...
<tr><td><%= Input.html(transaction.get("payment_date")) %></td><td><%= Input.html(transaction.get("payment_type")) %></td><td><span class="status-badge"><%= Input.html(transaction.get("status")) %></span></td><td><%= Input.html(transaction.get("reference_number")) %>
<% if ("TRANSFER".equals(transaction.get("payment_type")) && "COMPLETED".equals(transaction.get("status")) && "OUT".equals(transaction.get("direction")) && transaction.get("reference_number") != null) { %>
<br><a href="<%= request.getContextPath() %>/customer/transfer/receipt?reference=<%= java.net.URLEncoder.encode(transaction.get("reference_number").toString(), java.nio.charset.StandardCharsets.UTF_8) %>">Download Receipt</a>
<% } %></td><td><%= Input.html(transaction.get("recipient")) %></td><td><%= "BILL_PAYMENT".equals(transaction.get("payment_type")) && transaction.get("bill_reference")!=null?Input.html(transaction.get("bill_reference")):"Not applicable" %></td><td class="transaction-value"><%= "IN".equals(transaction.get("direction")) ? "+" : "-" %> LKR <%= Input.html(transaction.get("amount")) %></td></tr>
<% } %></tbody></table>
